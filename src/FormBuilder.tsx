...
const PATH_SETTINGS_STORAGE_KEY = "cv_task_path_settings_v4";
const TAURI_REQUIRED_MSG =
  "載入失敗：找不到 Tauri 介面，請透過 Tauri 視窗啟動（npm run tauri dev）";

const FormBuilder: React.FC<Props> = ({ tasks, onReload }) => {
  ...
  useEffect(() => {
    try {
      ...
      if (!raw) return;
      const parsed = JSON.parse(raw) as Record<string, TaskPathSettings>;
      setPathSettings(parsed);
    } catch {
  ...
  useEffect(() => {
    ...
    setPathSettings((prev) => {
      ...
      const changed =
        next.import_path !== existing.import_path ||
        next.export_path !== existing.export_path;
      ...
      try {
        ...
        for (const input of selected.inputs ?? []) {
          ...
          if (input.options_source) {
            const options = await invoke<SelectOption[]>("load_task_options", {
              taskId: selected.id,
              task_id: selected.id,
  ...
  useEffect(() => {
    ...
    const bind = async () => {
      try {
        ...
        if (active) setError(`監聽事件失敗：${String(e)}`);
      }
    };
  ...
  const validatePathSettings = (settings: TaskPathSettings): string | null => {
    ...
    if (importPath && !isValidPathFormat(importPath)) {
      return "匯入路徑格式不符";
    }
    if (exportPath && !isValidPathFormat(exportPath)) {
      return "匯出路徑格式不符";
    }
  ...
  const browsePathSetting = async (key: keyof TaskPathSettings) => {
    try {
      const current = (effectivePathSettings[key] ?? "").trim();
  ...
  const runTask = async () => {
    ...
    try {
      ...
      const pid = await invoke<number>("run_task_cmd", {
        taskId: selected.id,
        task_id: selected.id,
        form,
  ...
  const stopTask = async () => {
    ...
    try {
      ...
      await invoke("stop_task", { pid: runningPid });
      setBusyRun(false);
    } catch (e) {
  ...
  const runDiagnostics = async () => {
    ...
    try {
      ...
      const report = await invoke<DiagnosticsReport>("run_diagnostics_cmd");
  ...
  const openTemplate = async () => {
    ...
    try {
      ...
      const content = await invoke<string | null>("load_task_template", {
        taskId: selected.id,
        task_id: selected.id,
      });
...
function isValidPathFormat(value: string): boolean {
  const v = value.trim();
  if (!v) return true;
